...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    whateverMenu = { food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} };
    ...
    expect(createMenu(whateverMenu)['fetchMenu'] !== undefined).toBeTruthy();
    expect(typeof (createMenu(whateverMenu)['fetchMenu'])).toBe('function');
    ...
    // ```
    expect(createMenu(whateverMenu)['consumption'].length === 0).toBeTruthy();
